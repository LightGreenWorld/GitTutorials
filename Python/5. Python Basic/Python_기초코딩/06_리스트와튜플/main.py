#튜플&리스트
#0부터 1씩 증가하는 인덱스 번호로 관리한다.
#튜플: 다수의 기억 장소를 묶어서 관리하는 개념
#튜플은 성능이 좋지만 수정, 삭제, 추가 불가능
#리스트: 다수의 기억장소를 묶어서 관리하는 개념
#리스트는 성능이 덜 좋지만 수정, 삭제, 추가 가능
...
#인덱스
#양수 인덱스:  0부터 1씩 증가, 좌측부터 시작
#음수 인덱스: -1부터 1씩 감소, 우측부터 시작
...
#인덱스 n번째 값을 내보내고 리스트에서 삭제한다.
a2 = list200.pop(2)
print('a2:', a2)
print('list200의 개수:', len(list200))
print('list200:', list200)

#값을 지정해서 제거한다. 만약 같은 값이 여러개라면 제일 앞의 값을 제거한다.
list200.remove(20)
print('list200의 개수:', len(list200))
print('list200:', list200)
...
#위치반환
#지정된 값의 인덱스 번호를 반환한다. 만약 같은 갑이 여러개 라면
#제일 앞에 있는 값의 인덱스가 반환된다.
idx1 = list200.index(100)
print('idx1:', idx1)

#존재하지 않는 값을 찾으면 오류가 발생한다.
...
#리스트,튜플, 문자열 등을 + 로 연산하면 두 개를 합친 새로운 요소를 만든다.
...
#list300에 list400이 가지고 있는 값들을 추가한다.
...
list700.sort()
print('오름차순 정렬:', list700)

list700.sort(reverse=True)
print('내림차순 정렬:', list700)
