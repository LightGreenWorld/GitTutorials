...
# 객체가 가지고 있는 변수를 멤버라고 부른다.
# .연산자를 이용하면 객체가 가지고 있는 멤버에 접근할 수 있다.
# 만약 값을 넣어주는 코드를 작성했는데 해당 멤버가 있다면 덮어씌워주고,
# 해당 멤버가 없다면 새로 만들어준다 / 추가해준다.
t1.v1 = 100
t1.v2 = '안녕하세요'
print(f't1.v1 : {t1.v1}')
print(f't1.v2 : {t1.v2}')
...
# 생성자 함수
class TestClass2 :
    # __init__: 클래스를 통해 객체(인스턴스)가 생성된 후에 자동으로 호출되는 함수
    # 클래스에 정의한 모든 함수는 첫번째 매개변수로 self를 갖고 있다.
    # self에는 함수를 호출할 때 사용한 객체가 들어온다.
